// Package tarabish implements the rules of the game
package tarabish
...
// New Tarabish
func New(playerStore PlayerWriter, gameStore GameReadWriter) *Tarabish {
	fmt.Println("Start Tarabish")
...
func (s *Tarabish) AddPlayerToGame(playerName string, gameName string) (e error) {
...
func (s *Tarabish) AddObserverToGame(playerName string, gameName string) (handViews map[string]HandPlayerView, e error) {
	g, gfound := s.Games[gameName]
	if !gfound {
		e = fmt.Errorf("There is no Game with name %v", gameName)
		return
...
func (s *Tarabish) Play(pName string, cardPlayed deck.Card, cardsTaken []deck.Card) (handViews map[string]HandPlayerView,
...
func (s *Tarabish) Close(gName string, playerClosing string) {
	g := s.Games[gName]
