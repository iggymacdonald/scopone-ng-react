...
func newTestGameFactory(tarabish *Tarabish, gName string) *Game {
	return newGame("Player_1", "Player_2", "Player_3", "Player_4", tarabish, gName)
}
func newGame(p1 string, p2 string, p3 string, p4 string, tarabish *Tarabish, gName string) *Game {
	g, _ := tarabish.NewGame(gName)
	tarabish.PlayerEnters(p1)
	tarabish.PlayerEnters(p2)
	tarabish.PlayerEnters(p3)
	tarabish.PlayerEnters(p4)
	err_ := tarabish.AddPlayerToGame(p1, gName)
	if err_ != nil {
		panic(err_)
	}
	err_ = tarabish.AddPlayerToGame(p2, gName)
	if err_ != nil {
		panic(err_)
	}
	err_ = tarabish.AddPlayerToGame(p3, gName)
	if err_ != nil {
		panic(err_)
	}
	err_ = tarabish.AddPlayerToGame(p4, gName)
	if err_ != nil {
		panic(err_)
	}
	return g
...
func TestNewDeck(t *testing.T) {
	...
	tarabish := New(&DoNothingStore{}, &DoNothingStore{})
	firstPlayerSecondTeam := "Player_3"
	g := newGame("Player_1", "Player_2", firstPlayerSecondTeam, "Player_4", tarabish, "TestPlayCard")
	// fmt.Println(g.Teams[0].Players[0].Name)
	// fmt.Println(g.Teams[1].Players[0].Name)
	// fmt.Println(g.Teams[0].Players[1].Name)
	// fmt.Println(g.Teams[1].Players[1].Name)
	tarabish.NewHand(g)
	for _, player := range g.Players {
		if len(player.Cards) > 0 {
			t.Errorf("Expected %v but found %v; not as expected", 0, len(player.Cards))
		}
	}
	hand := currentHand(g)
	...
	tarabish.Shuffle(currentDealer(g).Name, g.Name)
	println("\nafter shuffle...")
	for _, card := range hand.Deck {
		fmt.Printf("%s of %s,", card.Type, card.Suit)
	}
	// dealer := currentDealer(g)
	// fmt.Printf("Dealer is %s\n", dealer.Name)
	// player := currentPlayer(g)
	// fmt.Printf("Current player is %s\n", player.Name)
	// playerSequence := playersSequence(g)
	// nextPlayer := playerSequence[dealer.Name]
	// if nextPlayer != player {
	// 	t.Errorf("Next Player expected is %v but found %v; not as expected", player, nextPlayer)
	// }
	// fmt.Println("state is currently", g.State)
	// hand := currentHand(g)
	// player := currentPlayer(g)
...
func TestPlayCard(t *testing.T) {
	...
	handView, _, _ := tarabish.Play(player.Name, card, []deck.Card{})
...
func TestPlayCardWithScopa(t *testing.T) {
	tarabish := New(&DoNothingStore{}, &DoNothingStore{})
	g := newGame("Player_1", "Player_2", "Player_3", "Player_4", tarabish, "TestPlayCardWithScopa")
	tarabish.NewHand(g)
	hand := currentHand(g)
	...
	tarabish.Play(player1.Name, card1, []deck.Card{})
	// player2 palys a card to make Scopa on the card played by player1
	handView, _, _ := tarabish.Play(player2.Name, card2, []deck.Card{card1})
...
// the rules of Tarabish are not followed in this test, all cards on the table are just taken, regardless of the card played
...
func TestPlayAllCards(t *testing.T) {
	tarabish := New(&DoNothingStore{}, &DoNothingStore{})
	g := newGame("Player_1", "Player_2", "Player_3", "Player_4", tarabish, "TestPlayAllCards")
	tarabish.NewHand(g)
	hand := currentHand(g)
	...
	for range hand.Deck {
		...
		handView, _, _ = tarabish.Play(player.Name, c, cardsTaken)
		cardOfLastPlayer = c
	}
...
func TestPlayAllCardsTwoHands(t *testing.T) {
	tarabish := New(&DoNothingStore{}, &DoNothingStore{})
	g := newGame("Player_1", "Player_2", "Player_3", "Player_4", tarabish, "TestPlayAllCardsTwoHands")
	// First Hand
	tarabish.NewHand(g)
	hand := currentHand(g)
	var cardOfLastPlayer deck.Card
	for range hand.Deck {
		...
		tarabish.Play(player.Name, c, cardsTaken)
		cardOfLastPlayer = c
	}
	// Second Hand
	tarabish.NewHand(g)
...
func TestNewHand(t *testing.T) {
	...
	for k := range handPlayersView {
		...
		// Test that the hand views for the players have no TarabishScope
...
func TestSortForPrimiera(t *testing.T) {
	...
	s.NewHand(g)
	hand := currentHand(g)
	// take all diamond from the shuffled deck
	var diamond byPrimiera = cardsWithSuit(deck.Diamond, hand.Deck)
	// order diamond for primiera
	sort.Sort(diamond)

	// there are 10 diamond
	if len(diamond) != 10 {
		t.Errorf("Number of cards is %v and not 10 as expected", len(diamond))
	}
	// the first one is seven
	if diamond[0].Type != "Seven" {
		t.Errorf("The first in Primiera is Seven but here we find %v", diamond[0])
	}
	// the second one is six
	if diamond[1].Type != "Six" {
		t.Errorf("The second in Primiera is Six but here we find %v", diamond[1])
	}
	// the third one one is Ace
	if diamond[2].Type != "Ace" {
		t.Errorf("The third in Primiera is Ace but here we find %v", diamond[2])
	}
...
func TestSortForPrimieraTwoSuits(t *testing.T) {
	...
	var diamond byPrimiera
	var bastoni byPrimiera
	var spade byPrimiera
	var coppe byPrimiera
	diamond = cardsWithSuit(deck.Diamond, hand.Deck)
	bastoni = cardsWithSuit("Heart", hand.Deck)
	...
	// there are 10 diamond
	if len(diamond) != 10 || len(bastoni) != 10 || len(spade) != 10 || len(coppe) != 10 {
		t.Errorf("Number of cards is %v and not 10 as expected", len(diamond))
	}
	// the first one is seven
	if diamond[0].Type != "Seven" || bastoni[0].Type != "Seven" || spade[0].Type != "Seven" || coppe[0].Type != "Seven" {
		t.Errorf("The first in Primiera is Seven but here we find %v", diamond[0])
	}
	// the second one is six
	if diamond[1].Type != "Six" || bastoni[1].Type != "Six" || spade[1].Type != "Six" || coppe[1].Type != "Six" {
		t.Errorf("The second in Primiera is Six but here we find %v", diamond[1])
	}
	// the third one one is Ace
	if diamond[2].Type != "Ace" || bastoni[2].Type != "Ace" || spade[2].Type != "Ace" || coppe[2].Type != "Ace" {
		t.Errorf("The third in Primiera is Ace but here we find %v", diamond[2])
	}
...
func TestSortForNapoli(t *testing.T) {
	...
	hand := currentHand(g)
	// take all diamond from the shuffled deck
	var diamond byNapoli = cardsWithSuit(deck.Diamond, hand.Deck)
	// order diamond for napoli
	sort.Sort(diamond)
	...
	if diamond[0].Type != "Ace" {
		t.Errorf("The first in Napoli is Ace but here we find %v", diamond[0])
	}
	// the second one is two
	if diamond[1].Type != "Two" {
		t.Errorf("The second in Napoli is Two but here we find %v", diamond[1])
	}
	// the third one one is Three
	if diamond[2].Type != "Three" {
		t.Errorf("The third in Napoli is Three but here we find %v", diamond[2])
	}
...
func TestPrimieraScore(t *testing.T) {
	d := []deck.Card{
		{Type: "Seven", Suit: deck.Diamond}, // 21
		{Type: "Five", Suit: deck.Diamond},
		{Type: "Ace", Suit: deck.Diamond}, // 18
		{Type: "Six", Suit: "Heart"},
		{Type: "Five", Suit: "Heart"},
		{Type: "Ace", Suit: "Heart"},
...
func TestCalculateScore(t *testing.T) {
	...
	takenCards1 := []deck.Card{
		{Type: "Seven", Suit: deck.Diamond}, // settebello    Denaro
		{Type: "Five", Suit: deck.Diamond},  //               Denaro
		{Type: "Ace", Suit: deck.Diamond},   //               Denaro
		{Type: "Six", Suit: "Heart"},
		{Type: "Five", Suit: "Heart"},
		{Type: "Ace", Suit: "Heart"},
		{Type: "Ace", Suit: "Spade"},
	...
	// This teams scores Carte, Diamond and Primiera
...
func TestCalculateScoreWithScopeAndNapoli(t *testing.T) {
	...
	takenCards1 := []deck.Card{
		{Type: "Seven", Suit: deck.Diamond}, // settebello    Denaro
		{Type: "Five", Suit: deck.Diamond},  //               Denaro
		{Type: "Ace", Suit: deck.Diamond},   //               Denaro - Napoli
		{Type: "Three", Suit: deck.Diamond}, //               Denaro - Napoli
		{Type: "Two", Suit: deck.Diamond},   //               Denaro - Napoli
		{Type: "Six", Suit: "Heart"},
		{Type: "Five", Suit: "Heart"},
		{Type: "Ace", Suit: "Heart"},
		{Type: "Ace", Suit: "Spade"},
	...
	// This teams scores Carte and Primiera - Diamond are five so no point is scored for it
	takenCards2 := deck.RemoveCards(hand.Deck, takenCards1)
	g.Teams[1].TakenCards = takenCards2
...
func TestPlayerEnters(t *testing.T) {
	...
	hv, alreadyIn := tarabish.PlayerEnters(playerName)
	if alreadyIn {
...
func TestNewGame(t *testing.T) {
	...
	_, err := tarabish.NewGame(gameName)
	if err != nil {
...
func TestAddPlayersToGame(t *testing.T) {
	...
	tarabish := New(&DoNothingStore{}, &DoNothingStore{})
	_, err_ := tarabish.NewGame(gameName)
	if err_ != nil {
		panic(err_)
	}
	g := tarabish.Games[gameName]
	for _, name := range playerNames {
		tarabish.PlayerEnters(name)
		e := tarabish.AddPlayerToGame(name, gameName)
		if e != nil {
		...
		e = tarabish.AddPlayerToGame(playerName1, gameName)
		if e == nil {
		...
		pFromOsteria := tarabish.Players[playerName1]
	...
	tarabish.PlayerEnters(playerName5)
	e := tarabish.AddPlayerToGame(playerName5, gameName)
	if e == nil {
	...
	e = tarabish.AddPlayerToGame(playerName5, "a game that does not exist")
	if e == nil {
	...
	e = tarabish.AddPlayerToGame("a player that does not exist", gameName)
	if e == nil {
...
func TestAddPlayerToOsteriaAndThenRemove(t *testing.T) {
	...
	tarabish.PlayerEnters(playerName)
	tarabish.RemovePlayer(playerName)
	...
	pStatus := tarabish.Players[playerName].Status
...
func TestAddPlayerToGameAndThenRomove(t *testing.T) {
	...
	err_ = tarabish.AddPlayerToGame(playerName, gameName)
	if err_ != nil {
		panic(err_)
	}
...
func TestStartGameAndThenRemovePlayers(t *testing.T) {
	...
	for _, name := range playerNames {
		tarabish.PlayerEnters(name)
		err_ := tarabish.AddPlayerToGame(name, gameName)
		if err_ != nil {
			panic(err_)
		}
	}
	...
	tarabish.RemovePlayer(playerName3)
	...
	tarabish.PlayerEnters(playerName3)
	...
	tarabish.RemovePlayer(playerName1)
	tarabish.RemovePlayer(playerName2)
	tarabish.RemovePlayer(playerName3)
	tarabish.RemovePlayer(playerName4)
	tarabish.PlayerEnters(playerName2)
	if g.State != GameSuspended {
		t.Errorf("Game \"%v\" should be suspended but is in state %v", gameName, g.State)
	}
	// add 2 more players and the game is still suspended
	tarabish.PlayerEnters(playerName3)
	tarabish.PlayerEnters(playerName4)
	if g.State != GameSuspended {
		t.Errorf("Game \"%v\" should be suspended but is in state %v", gameName, g.State)
	}
	// add the last one and the game is open again
	tarabish.PlayerEnters(playerName1)
	if g.State != GameOpen {
		t.Errorf("Game \"%v\" should be open but is in state %v", gameName, g.State)
	}
